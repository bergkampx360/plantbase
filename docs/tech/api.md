...
`{ chunks, hitCount, topScore }` alakú JSON-t ad vissza:

- `chunks`: a top-N (jelenleg 4) legrelevánsabb szövegrészlet (`source`, `title`, `category`,
  `content`, `score`).
- `hitCount` / `topScore`: az agent önreflektáló keresésének alapja — a system prompt szabálya
  szerint gyenge (`topScore` alacsony vagy `hitCount` 0) találatnál az agent egyszer
  újrafogalmazott kérdéssel újrahívhatja a toolt, mielőtt kimondaná, hogy "nincs erről infóm".
