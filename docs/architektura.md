...
2. **Két DB-kapcsolat, két jog — J1-től (HF5, `docs/implementation/09-customer-facing-poc.md`) egy harmadik, szűk kivétellel.** Az agent `runSql`-je READ-ONLY kapcsolaton fut (`DATABASE_URL_READONLY`), csak SELECT. A Prisma READ-WRITE kapcsolaton (`DATABASE_URL`) viszi a sémát, migrációt, seedet. Az agent NEM Prismán kérdez.
   ...
   **J1-től**: egyetlen, additív kivétel ez alól — a customer-facing agent `requestHumanHandoff`
   toolja egy harmadik, `DATABASE_URL_HANDOFF` kapcsolaton, kizárólag INSERT-et végezhet a
   `customer_handoffs` táblára (SELECT-et sem lát, még saját magát sem tudja visszaolvasni). A
   meglévő `plantbase_ro` SELECT-je erre a táblára explicit REVOKE-olva van (különben a
   szerepkört létrehozó `ALTER DEFAULT PRIVILEGES` automatikusan SELECT-hozzáférést adott volna
   rá a belső, szabad-SQL-es `runSql` toolnak is). A jóváhagyás/elutasítás (`status` UPDATE)
   sosem az agent útján megy, hanem `apps/server`-ből, a Prisma RW kapcsolaton — ember végzi.
   Részletek: `docs/tech/infra.md` "Három DB-kapcsolat, három jog".
